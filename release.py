#!/usr/bin/env python3
"""
Release script for HEIF Converter.
...
def get_current_version():
    ...
    with open('hif2jpegUI.py', 'r') as f:
        content = f.read()
        match = re.search(r"self\.root\.title\(\"HEIF to JPEG Converter v([^\"]+)\"\)", content)
        if match:
...
def update_version_in_file(file_path, new_version):
    ...
    content = re.sub(
        r"(self\.root\.title\(\"HEIF to JPEG Converter)( v[^\"]+)?(\"\))",
        f"\\1 v{new_version}\\3",
        content
    )
...
def main():
    parser = argparse.ArgumentParser(description='Create a new release of HEIF Converter')
    ...
    update_version_in_file('hif2jpegUI.py', new_version)
    print(f"Updated version in hif2jpegUI.py")
    ...
    # Commit changes
    run_command(f'git add hif2jpegUI.py CHANGELOG.md')
