...
class ModernTooltip:
    ...
    def show_tooltip(self, event=None):
        try:
            ...
            # Widget doesn't support bbox("insert"), use widget position instead
            x = self.widget.winfo_rootx() + 25
    ...
    def hide_tooltip(self, event=None):
        ...
        if self.fade_after_id:
            self.widget.after_cancel(self.fade_after_id)
            self.fade_after_id = None
    ...
    def fade_in(self, alpha=0.0):
        if self.tooltip_window:
            if alpha < 1.0:
                ...
                self.fade_after_id = self.widget.after(20, self.fade_in, alpha + 0.1)
            else:
                self.fade_after_id = None
